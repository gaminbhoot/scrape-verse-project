...
test('API Contracts - SQL/Script Injection Payloads in Parameters', () => {
  ...
  assert.strictEqual(
    ...
    'Neutralizes SQL injection payloads'
  );
  assert.strictEqual(
    sanitizedXss,
    'alert(1)</script>Collector',
    'Neutralizes script injection payloads'
  );
});
...
test('API Contracts - MTTR (Mean Time to Recovery) Latency Thresholds', async () => {
  ...
  assert.ok(
    latencyMs < 1500,
    `MTTR latency (${latencyMs.toFixed(2)}ms) exceeded the 1500ms threshold requirement`
  );
});
