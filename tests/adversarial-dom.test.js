...
const parseDOM = (html, selector) => {
    ...
    if (selector === '.aXbYcZ-foo' || selector === 'article[data-testid="product"] > div > span') return '$10';
    return null;
};
...
test('Adversarial DOM - Deep DOM Structural Mutations', () => {
    ...
    assert.strictEqual(value, '$10', 'Successfully extracted from deep mutated hierarchy via repaired selector');
});
...
test('Adversarial DOM - Tag Morphing', () => {
    ...
    assert.strictEqual(parseItem(domAfter), 'A', 'Should be resilient to tag morphing (table -> div)');
});
