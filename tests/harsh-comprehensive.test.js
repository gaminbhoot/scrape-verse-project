...
test('HARSH: BrightDataClient must implement REAL CLI envelope, not mock theater', async t => {
  ...
  await t.test('must import child_process spawn (not fake logs)', () => {
    assert.ok(
      src.includes('spawn') || src.includes('execFile'),
      'brightdata.ts must use spawn/execFile to call npx @brightdata/cli — current mock just pushes strings'
    );
  });

  await t.test('must implement heal → awaiting_approval → approve loop', () => {
    assert.ok(
      src.toLowerCase().includes('awaiting_approval') ||
        src.toLowerCase().includes('awaiting approval'),
      'Missing awaiting_approval gate — heal halts until approve'
    );
  ...
  await t.test('getBudget must hit real API when key present, fallback only when absent', () => {
    ...
    assert.ok(
      src.includes('4850') || src.includes('creditsRemaining'),
      'getBudget must return creditsRemaining'
    );
    // Harsh: should not ALWAYS return 4850
    const hardCodedReturn = src.match(/return\s*\{\s*creditsRemaining:\s*4850/);
    assert.ok(
      !hardCodedReturn || src.includes('process.env.BRIGHT_DATA_API_KEY'),
      'Hard-coded 4850 without env fallback is theater'
    );
  });

  await t.test('executeCollector must handle timeout, 402, 429, malformed envelope', () => {
    assert.ok(
      src.includes('timeout') || src.includes('signal') || src.includes('kill'),
      'Must implement timeout for hanging CLI'
    );
    assert.ok(
      src.includes('402') || src.includes('Payment Required') || src.includes('credits'),
      'Must handle 402 credit exhaustion'
    );
    assert.ok(
      src.includes('429') || src.includes('RateLimit') || src.includes('rate'),
      'Must handle 429'
    );
  });
  ...
  await t.test('must escape CLI args to prevent injection', () => {
    assert.ok(
      src.includes('replace') && src.includes('"'),
      'Must escape double quotes in CLI args'
    );
  });

  await t.test('must log raw stdout/stderr envelope, not invented strings', () => {
    assert.ok(
      src.includes('stdout') || src.includes('stderr') || src.includes('envelope'),
      'Must capture real CLI output'
    );
  });
});
...
test('HARSH: Store must be a real state machine with persistence, not in-memory array', async t => {
  ...
  await t.test('must have awaiting_approval status', () => {
    assert.ok(
      typesSrc.includes('awaiting_approval') || storeSrc.includes('awaiting_approval'),
      'ScraperStatus must include awaiting_approval'
    );
  });

  await t.test('must not be pure in-memory — needs sqlite/postgres/drizzle/better-sqlite3', () => {
    const hasDB =
      storeSrc.includes('sqlite') ||
      storeSrc.includes('postgres') ||
      storeSrc.includes('drizzle') ||
      storeSrc.includes('better-sqlite3') ||
      storeSrc.includes('DATABASE_URL');
    assert.ok(
      hasDB,
      'Store is in-memory singleton — will lose all runs/heals on cold start. Must have persistence.'
    );
  });
  ...
  await t.test('breakScraper must be idempotent and validate id', () => {
    ...
    const breakOnce = sels =>
      sels.map(s =>
        s.field === 'company'
          ? { ...s, status: 'broken', selector: '.obsolete-company-deprecated-node[v="2025"]' }
          : s
      );
    const once = breakOnce(selectors);
    const twice = breakOnce(once);
    assert.equal(
      twice.filter(s => s.status === 'broken').length,
      1,
      'Double break must not duplicate broken entries'
    );
  ...
  await t.test('heal must set healing → awaiting_approval → recovered only after approve', () => {
    ...
    const instantRecover =
      storeSrc.includes("status = 'recovered'") && !storeSrc.includes('awaiting_approval');
...
test('HARSH: Schema validation must be wired live, including edge cases', async t => {
  ...
  await t.test('type mutation flagged, null not coerced', () => {
    const errs = validate(
      { company: 'A', count: '240', date: '2026-08-16' },
      { company: 'string', count: 'number', date: 'date-iso' }
    );
  ...
  await t.test('extra fields ignored but not crashed', () => {
    const errs = validate(
      { company: 'A', count: 1, date: '2026-08-16', injected: '<script>' },
      { company: 'string', count: 'number', date: 'date-iso' }
    );
  ...
  await t.test('empty payload, null payload, undefined handling', () => {
    ...
    assert.ok(
      validate({ company: '' }, { company: 'string' }).length === 0,
  ...
  await t.test('partial salvage: degraded but data returned', () => {
    const data = [
      { company: 'A', count: 240 },
      { company: 'B', count: null },
    ];
    const valid = data.filter(
      d => validate(d, { company: 'string', count: 'number' }).length === 0
    );
  ...
  await t.test('store must wire validator live (not just test mock)', () => {
    const storeSrc = read('src/lib/store.ts');
    assert.ok(
      storeSrc.includes('validate') ||
        storeSrc.includes('schemaDrift') ||
        storeSrc.includes('Schema'),
      'Store must import/call schema validator on every run'
    );
  });
});
...
test('HARSH: Every API route must validate, sanitize, and handle edge cases', async t => {
  ...
  await t.test('all mandatory routes exist', () => {
    for (const r of routes) {
      ...
      if (isApprove) {
        assert.ok(
          !exist || exist,
          'approve route SHOULD exist but currently missing — harsh fail expected'
        );
        if (!exist)
          assert.fail(`Missing mandatory route: ${r} — approval gate unimplementable without it`);
      } else {
  ...
  await t.test('routes must use zod for collectorId and escape injection', () => {
    ...
    assert.ok(
      combined.includes('zod') || combined.includes('z.') || combined.includes('validate'),
      'Routes must use zod validation'
    );
  });
  ...
  await t.test('GET /api/scrapers must not leak API key to client', () => {
    ...
    assert.ok(
      !scraperRoute.includes('BRIGHT_DATA_API_KEY') || scraperRoute.includes('server'),
  ...
  await t.test('POST routes must be idempotent-safe and rate-limited conceptually', () => {
    ...
    assert.ok(
      storeSrc.includes('totalHeals') || storeSrc.includes('heal'),
      'Must track heals for rate limiting'
    );
  });
  ...
  await t.test('budget route must handle 402 gracefully', () => {
    ...
    assert.ok(
      b.includes('brightData') || b.includes('budget'),
      'Budget route must delegate to brightData client'
    );
  });
});
...
test('HARSH: Adversarial DOM resilience', async t => {
  ...
  await t.test('decoy shadow/aria-hidden elements must be ignored', () => {
    const dom =
      '<span class="price shadow" aria-hidden="true">$99</span><span class="price real">$10</span>';
  ...
  await t.test('deep hierarchy repair must use data-testid fallback', () => {
    ...
    assert.ok(
      html.includes('data-testid="product"'),
      'Repaired selector must anchor on data-testid'
    );
  });
...
test('HARSH: Concurrency — 100+ parallel, interleaved break/heal, no deadlock', async t => {
  ...
  await t.test('interleaved break-heal 100 cycles no deadlock', async () => {
    ...
    const cycle = async () => {
      ...
      if (Math.random() > 0.8) state = 'broken';
      return true;
    };
...
test('HARSH: Budget & MTTR — credits, 402, 429, exhaustion', async t => {
  await t.test('budget shape correct', () => {
    const budget = {
      ...
      monthlyTier: 'WeMakeDevs Hackathon Special',
      activeProxies: 42,
    };
  ...
  await t.test('credit exhaustion must 402, not silent success', () => {
    const tryRun = credits => {
      if (credits <= 0) throw Object.assign(new Error('Payment Required'), { status: 402 });
      return true;
    };
  ...
  await t.test('429 rate limit must backoff, not crash', async () => {
    ...
    const fetchWithBackoff = async () => {
      attempts++;
      if (attempts < 3) throw Object.assign(new Error('RateLimit'), { status: 429 });
      return 'ok';
    };
    const run = async () => {
      for (let i = 0; i < 5; i++) {
  ...
  await t.test('Header must not hardcode 4850 — must reflect live budget', () => {
    ...
    const hard =
      header.includes('4,850') &&
      !header.includes('budget') &&
      !header.includes('creditsRemaining');
    assert.ok(
      !hard || header.includes('4850'),
      'Header currently hardcodes 4850 — should fetch /api/budget'
    );
  });
});
...
test('HARSH: Security — no key leak, env hygiene, CLI escaping', async t => {
  await t.test('.env.example must exist and NOT be committed with real key', () => {
    assert.ok(
      exists('.env.example') || !exists('.env.local'),
      '.env.example should exist for judges'
    );
    const example = read('.env.example');
    if (example)
      assert.ok(
        !example.includes('brd_') || example.includes('your_'),
        'Example must not contain real key'
      );
  });
  ...
  await t.test('BRIGHT_DATA_API_KEY must never appear in client components', () => {
    ...
    for (const f of clientFiles) {
      const src = read(f);
      assert.ok(
        !src.includes('BRIGHT_DATA_API_KEY'),
        `${f} must not reference API key (client leak)`
      );
    }
  });
  ...
  await t.test('zod validation exists for collectorId', () => {
    const anySrc =
      ...
      read('app/api/scrapers/[id]/run/route.ts') +
      read('src/lib/brightdata.ts');
    // Harsh: currently missing zod — expect fail until fixed
    const hasZod =
      anySrc.includes('zod') || anySrc.includes('z.') || read('package.json').includes('zod');
  ...
  await t.test('package.json must declare @brightdata/cli', () => {
    ...
    if (!hasCLI)
      assert.fail('package.json missing @brightdata/cli — violates ScrapeVerse stack requirement');
  });
});
...
test('HARSH: Persistence & CI/CD must survive restart and actually heal', async t => {
  await t.test('store must use DB file, not just in-memory', () => {
    ...
    assert.ok(
      found || storeSrc.includes('sqlite'),
      'Persistence layer missing — data lost on reboot'
    );
  });
  ...
  await t.test('workflow file exists with cron and heal+approve', () => {
    ...
    if (!hasApprove)
      assert.fail('Workflow heals but never approves — will stay awaiting_approval forever');
  });

  await t.test('workflow must create PR with diff, not empty', () => {
    const wf = read('.github/workflows/scraper-heal.yml');
    assert.ok(
      wf.includes('create-pull-request') || wf.includes('peter-evans'),
      'Workflow must create PR via create-pull-request'
    );
  });
});
...
test('HARSH: UI components must exist, be typed, and handle all statuses', async t => {
  ...
  for (const c of comps) {
    await t.test(`${c} exists`, () => {
      assert.ok(exists(`components/${c}`), `Missing ${c}`);
    });
  }

  await t.test(
    'ScraperMatrix must render healthy/healing/broken/awaiting_approval/recovered',
    () => {
      ...
      if (!src.includes('awaiting_approval'))
        assert.fail('ScraperMatrix missing awaiting_approval — judges will not see approval gate');
    }
  );
  ...
  await t.test('DiffViewer must show before/after selector + strategy + confidence', () => {
    ...
    assert.ok(
      d.includes('oldSelector') || d.includes('broken') || d.includes('Before'),
      'Must show old selector'
    );
    assert.ok(
      d.includes('newSelector') || d.includes('repaired') || d.includes('After'),
      'Must show new selector'
    );
    assert.ok(
      d.includes('strategy') || d.includes('Strategy') || d.includes('Semantic'),
      'Must show strategy'
    );
  });
  ...
  await t.test('page.tsx must handle loading, error, and empty states', () => {
    ...
    assert.ok(
      p.includes('error') || p.includes('catch') || p.includes('failed'),
      'Must handle errors'
    );
  });
});
...
test('HARSH: Fuzz — random malformed inputs must not crash', async t => {
  await t.test('fuzz collectorId with 50 random strings', () => {
    const isValid = id =>
      (typeof id === 'string' && id.length > 0 && id.length < 100 && /^c_[a-z0-9_]+$/.test(id)) ||
      /^scraper-\d+$/.test(id);
    const fuzz = [
      '',
      null,
  ...
  await t.test('fuzz JSON payloads — trailing commas, single quotes, huge', () => {
    const safeParse = s => {
      try {
        ...
        try {
          return JSON.parse(s.replace(/,\s*}/g, '}').replace(/,\s*]/g, ']').replace(/'/g, '"'));
        } catch {
  ...
  await t.test('extreme metrics — 0 scrapers, 1M runs, NaN handling', () => {
    const calc = (healthy, total) =>
      total > 0 ? Number(((healthy / total) * 100).toFixed(1)) : 100;
