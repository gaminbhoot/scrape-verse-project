...
class ScraperStore {
  ...
  private logs: LogEntry[] = [
    {
      ...
      level: 'info',
      source: 'CLI',
      message: 'Initialized @brightdata/cli session. Authenticated with Collector Pool. Persistence: sqlite (better-sqlite3) via DATABASE_URL=file:./data/aegis.db',
    },
    {
      id: 'log-2',
    ...
    },
    {
      ...
      level: 'success',
      source: 'STUDIO',
      message: 'Collector c_llm_benchmarks_live synced data into SQLite store (better-sqlite3). 0 schema drift.',
      collectorId: 'c_llm_benchmarks_live',
    }
  ];
  ...
  getScrapers(): Scraper[] { return this.scrapers; }
  getScraper(id: string): Scraper | undefined { return this.scrapers.find(s => s.id === id); }
  getRuns(): ScraperRun[] { return this.runs; }
  getHealEvents(): HealEvent[] { return this.healEvents; }
  getLogs(): LogEntry[] {
    try {
      const db = getDb();
      if (db && this.logs.length < 5) {
        const rows = db.prepare('SELECT * FROM logs ORDER BY timestamp DESC LIMIT 50').all() as any[];
        if (rows.length > this.logs.length) {
          return rows.map(r => ({ id: r.id, timestamp: r.timestamp, level: r.level as any, source: r.source as any, message: r.message, collectorId: r.collectorId }));
        }
      }
    } catch {}
    return this.logs;
  ...
  getMetrics(): MetricOverview {
    ...
    const healing = this.scrapers.filter(s => s.status === 'healing' || s.status === 'awaiting_approval').length;
    return {
      totalScrapers: total,
  ...
  breakScraper(id: string): { scraper: Scraper; log: LogEntry } {
    ...
    if (scraper.status === 'broken') {
      const log: LogEntry = { id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'warn', source: 'ENGINE', message: `[DEMO BREAKER] Scraper ${scraper.collectorId} already broken — idempotent break ignored.`, collectorId: scraper.collectorId };
      this.logs.push(log);
      return { scraper, log };
    }
    ...
    scraper.selectors = scraper.selectors.map(s => {
      if (s.field === 'company' || s.field === 'count' || s.field === 'item') {
        return { ...s, status: 'broken', selector: `.obsolete-${s.field}-deprecated-node[v="2025"]` };
      }
      return s;
    });
    const log: LogEntry = { id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'error', source: 'ENGINE', message: `[DEMO BREAKER] Injected target website DOM mutation for ${scraper.collectorId}. 2 selectors broken.`, collectorId: scraper.collectorId };
    this.logs.push(log);
    return { scraper, log };
  }
  ...
  private validateSchema(data: any, schema: Record<string,string>): string[] {
    ...
    for (const [k, type] of Object.entries(schema)) {
      ...
      if (type === 'number' && typeof data[k] !== 'number') errs.push(`type:${k} expected number got ${typeof data[k]}`);
      if (type === 'string' && typeof data[k] !== 'string') errs.push(`type:${k} expected string got ${typeof data[k]}`);
  ...
  async runScraper(id: string): Promise<ScraperRun> {
    ...
    if (result.data.length > 0) {
      ...
      drift = errs.length > 0;
      if (drift) {
        this.logs.push({ id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'warn', source: 'ENGINE', message: `[SCHEMA DRIFT] ${scraper.collectorId}: ${errs.join('; ')} — partial data salvaged`, collectorId: scraper.collectorId });
      }
    } else {
    ...
    const runLog: LogEntry = {
      ...
      source: 'CLI',
      message: result.success ? `Run completed for ${scraper.collectorId}: ${result.data.length} records extracted. stdout=${(result.stdout||'').slice(0,80)} stderr=${(result.stderr||'').slice(0,80)} exitCode=${result.exitCode}` : `Run failed for ${scraper.collectorId}: ${result.error} (exitCode=${result.exitCode})`,
    ...
    try {
      const db = getDb();
      if (db) {
        db.prepare('INSERT OR REPLACE INTO runs (id, scraperId, timestamp, status, durationMs, recordsExtracted, sampleData, schemaDriftDetected, logs, error) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(run.id, run.scraperId, run.timestamp, run.status, run.durationMs, run.recordsExtracted, JSON.stringify(run.sampleData), run.schemaDriftDetected ? 1 : 0, JSON.stringify(run.logs), run.error || null);
        db.prepare('INSERT OR REPLACE INTO logs (id, timestamp, level, source, message, collectorId) VALUES (?, ?, ?, ?, ?, ?)').run(runLog.id, runLog.timestamp, runLog.level, runLog.source, runLog.message, runLog.collectorId || null);
      }
    } catch {}
    return run;
  }

  async healScraper(id: string): Promise<{ healEvent: HealEvent; scraper: Scraper; awaitingApproval: boolean }> {
    const scraper = this.getScraper(id);
    if (!scraper) throw new Error(`Scraper ${id} not found`);
    if (this.healLocks.has(id)) throw new Error(`HealInProgress: scraper ${id} already healing/awaiting_approval`);
    if (scraper.status === 'healing' || scraper.status === 'awaiting_approval') throw new Error(`HealInProgress: scraper ${id} status=${scraper.status}`);
    this.healLocks.add(id);
    scraper.status = 'healing';
    this.logs.push({ id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'heal', source: 'HEALER', message: `Triggering automated self-healing via 'bdata scraper heal ${scraper.collectorId}'... stdout/stderr envelope will be captured`, collectorId: scraper.collectorId });
    const healResult = await brightData.healCollector(scraper, 'DOM structure mutated by target site; primary CSS selectors failing');
    ...
    const healEvent: HealEvent = {
      ...
      prGenerated: 'PR #14: fix(collector): self-healed selector mappings for c_layoffs_v2_hackathon (awaiting approval)',
    };
    this.healEvents.push(healEvent);
    this.pendingHeals.set(id, healEvent);
    const healLog: LogEntry = { id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'heal', source: 'HEALER', message: `Heal completed for ${scraper.collectorId} — status=awaiting_approval — awaiting bdata scraper approve ${scraper.collectorId} --auto-approve. diff_summary=${JSON.stringify(healResult.repairedSelectors).slice(0,300)} envelope=${JSON.stringify(healResult.envelope ?? {}).slice(0,300)}`, collectorId: scraper.collectorId };
    this.logs.push(healLog);
    try {
      const db = getDb();
      if (db) {
        db.prepare('INSERT OR REPLACE INTO heal_events (id, scraperId, timestamp, triggerReason, brokenSelectors, repairedSelectors, confidenceScore, timeToHealMs, cliCommandUsed, prGenerated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(healEvent.id, healEvent.scraperId, healEvent.timestamp, healEvent.triggerReason, JSON.stringify(healEvent.brokenSelectors), JSON.stringify(healEvent.repairedSelectors), healEvent.confidenceScore, healEvent.timeToHealMs, healEvent.cliCommandUsed, healEvent.prGenerated || null);
        db.prepare('INSERT OR REPLACE INTO logs (id, timestamp, level, source, message, collectorId) VALUES (?, ?, ?, ?, ?, ?)').run(healLog.id, healLog.timestamp, healLog.level, healLog.source, healLog.message, healLog.collectorId || null);
      }
    } catch {}
  ...
  async approveScraper(id: string): Promise<{ healEvent: HealEvent; scraper: Scraper; verifiedRun: ScraperRun }> {
    const scraper = this.getScraper(id);
    if (!scraper) throw new Error(`Scraper ${id} not found`);
    if (scraper.status !== 'awaiting_approval') throw new Error(`ApproveRejected: scraper ${id} not in awaiting_approval (current=${scraper.status})`);
    ...
    this.logs.push({ id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'heal', source: 'HEALER', message: `Approving heal for ${scraper.collectorId}: ${approveResult.logs.join(' | ').slice(0,500)}`, collectorId: scraper.collectorId });
    // Apply repaired selectors only on approve
    scraper.selectors = scraper.selectors.map(s => {
      const repaired = pending.repairedSelectors.find(r => r.field === s.field);
      if (repaired) return { ...s, status: 'repaired', selector: repaired.newSelector, repairedFrom: repaired.oldSelector };
    ...
    const verifyLog: LogEntry = { id: `log-${Date.now()}`, timestamp: new Date().toISOString(), level: 'success', source: 'CI/CD', message: `Self-healing verified! Collector ${scraper.collectorId} restored to 100% data fidelity in ${pending.timeToHealMs}ms. Verified run ${verifiedRun.id} with ${verifiedRun.recordsExtracted} records.`, collectorId: scraper.collectorId };
    this.logs.push(verifyLog);
    try {
      const db = getDb();
      if (db) {
        db.prepare('INSERT OR REPLACE INTO logs (id, timestamp, level, source, message, collectorId) VALUES (?, ?, ?, ?, ?, ?)').run(verifyLog.id, verifyLog.timestamp, verifyLog.level, verifyLog.source, verifyLog.message, verifyLog.collectorId || null);
      }
    } catch {}
