...
export class BrightDataClient {
  ...
  async triggerViaRest(collectorId: string, targetUrl: string): Promise<{ stdout: string; stderr: string; exitCode: number; envelope: any }> {
    ...
    if (!res.ok) {
      return { stdout: text, stderr: `REST ${res.status} ${res.statusText}: ${text}`, exitCode: res.status, envelope };
    }
    return { stdout: text, stderr: '', exitCode: 0, envelope };
  }

  private runCli(args: string[], timeoutMs = 90000): Promise<{ stdout: string; stderr: string; exitCode: number; envelope: any }> {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        killed = true;
        child.kill('SIGTERM');
        reject(Object.assign(new Error(`CLI timeout after ${timeoutMs}ms: npx @brightdata/cli ${args.join(' ')}`), { stdout, stderr, exitCode: 124 }));
      }, timeoutMs);
      ...
      child.on('close', (code) => {
        ...
        try {
          ...
          for (let i = lines.length - 1; i >= 0; i--) {
            try { envelope = JSON.parse(lines[i]); break; } catch {}
          }
  ...
  async executeCollector(scraper: Scraper, isBroken: boolean = false): Promise<{
    ...
    const cliCmd = formatCliCommand('npx @brightdata/cli bdata scraper run', [scraper.collectorId, scraper.targetUrl, '--format', 'json']);
    const logs: string[] = [];
    ...
    if (!this.apiKey && this.allowMock) {
      ...
      if (isBroken) {
        ...
        logs.push(`[STUDIO] Collector ${scraper.collectorId} exited with code 1 (Schema Mismatch / Null Payload) — stderr captured`);
        logs.push(`[ENVELOPE] {"status":"failure","exitCode":1,"error":"Target site DOM modification detected"}`);
        return {
          success: false,
          data: [],
      ...
      logs.push(`[DOM] Page rendered successfully. Extracting ${Object.keys(scraper.schema).length} target schema attributes...`);
      ...
      if (validation.length > 0) {
        logs.push(`[WARN] Schema drift: ${validation.join('; ')} — graceful degrade, partial data returned`);
      }
      return {
        success: true,
        data: mockData,
    ...
    try {
      ...
      const useRest = process.env.BRIGHT_DATA_USE_REST === 'true' || process.env.BRIGHT_DATA_USE_REST === '1';
      if (useRest && this.apiKey) {
        logs.push(`[REST] POST https://api.brightdata.com/dca/trigger?collector=${scraper.collectorId} {url:"${scraper.targetUrl}"}`);
        result = await this.triggerViaRest(scraper.collectorId, scraper.targetUrl);
      } else {
        result = await this.runCli(['bdata', 'scraper', 'run', scraper.collectorId, scraper.targetUrl, '--format', 'json'], 90000);
      }
      logs.push(`[STDOUT] ${result.stdout.slice(0, 2000)}`);
      if (result.stderr) logs.push(`[STDERR] ${result.stderr.slice(0, 2000)}`);
      logs.push(`[ENVELOPE] exitCode=${result.exitCode} ${result.envelope ? JSON.stringify(result.envelope).slice(0,500) : 'no envelope'}`);
      if (result.stderr.includes('402') || result.stderr.includes('Payment Required')) {
        return { success: false, data: [], durationMs: Date.now() - startTime, logs, error: '402 Payment Required — credits exhausted', envelope: result.envelope, exitCode: 402, stdout: result.stdout, stderr: result.stderr };
      }
      if (result.stderr.includes('429') || result.stderr.includes('RateLimit')) {
        return { success: false, data: [], durationMs: Date.now() - startTime, logs, error: '429 RateLimit — backoff required', envelope: result.envelope, exitCode: 429, stdout: result.stdout, stderr: result.stderr };
      }
      if (result.exitCode !== 0 || !result.envelope) {
        return { success: false, data: [], durationMs: Date.now() - startTime, logs, error: result.stderr || 'CLI failure without envelope', envelope: result.envelope, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr };
      }
      ...
      if (arrayData.length === 0) {
        return { success: false, data: [], durationMs: Date.now() - startTime, logs, error: 'Empty payload — schema mismatch / selectors failed', envelope: result.envelope, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr };
      }
      return { success: true, data: arrayData, durationMs: Date.now() - startTime, logs, envelope: result.envelope, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr };
    } catch (e: any) {
      logs.push(`[ERROR] CLI exception: ${e.message}`);
      if (e.stderr) logs.push(`[STDERR] ${e.stderr}`);
      return { success: false, data: [], durationMs: Date.now() - startTime, logs, error: e.message, envelope: null, exitCode: e.exitCode ?? 1, stdout: e.stdout ?? '', stderr: e.stderr ?? '' };
    }
  }
  ...
  async healCollector(scraper: Scraper, reason: string): Promise<{
    ...
    if (!this.apiKey && this.allowMock) {
      logs.push(`[DOM-DIFF] Detected updated container from div.card-v1 to article[data-testid="item-card"] — SIMULATION`);
      ...
      logs.push(`[ENVELOPE] {"status":"awaiting_approval","diff_summary":${JSON.stringify(repairedSelectors).slice(0,200)},"preview_result":{"confidence":98.4}}`);
      logs.push(`[HEALER] Heal completed — status=awaiting_approval — awaiting bdata scraper approve`);
      return {
        success: true,
        cliCommand,
        logs,
    ...
    try {
      const result = await this.runCli(['bdata', 'scraper', 'heal', scraper.collectorId, reason], 90000);
      logs.push(`[STDOUT] ${result.stdout.slice(0,2000)}`);
      if (result.stderr) logs.push(`[STDERR] ${result.stderr.slice(0,2000)}`);
      logs.push(`[ENVELOPE] ${JSON.stringify(result.envelope ?? {}).slice(0,1000)}`);
      const status = result.envelope?.status || (result.stdout.includes('awaiting_approval') ? 'awaiting_approval' : 'unknown');
      const awaitingApproval = status === 'awaiting_approval';
      if (awaitingApproval) logs.push(`[HEALER] Heal requires approval — run bdata scraper approve ${scraper.collectorId} --auto-approve`);
      const repaired = result.envelope?.diff_summary ?? result.envelope?.repairedSelectors ?? [];
      const mapped = Array.isArray(repaired) ? repaired.map((r: any) => ({
      ...
      return { success: false, cliCommand, logs, repairedSelectors: [], timeToHealMs: Date.now() - startTime, envelope: null, awaitingApproval: false, stdout: e.stdout ?? '', stderr: e.stderr ?? '' };
    }
  }

  async approveCollector(collectorId: string): Promise<{ success: boolean; logs: string[]; envelope: any; stdout: string; stderr: string }> {
    ...
    if (!this.apiKey && this.allowMock) {
      ...
      return { success: true, logs, envelope: { status: 'approved' }, stdout: logs.join('\n'), stderr: '' };
    }
    try {
      const result = await this.runCli(['bdata', 'scraper', 'approve', collectorId, '--auto-approve'], 60000);
      logs.push(`[STDOUT] ${result.stdout.slice(0,2000)}`);
      if (result.stderr) logs.push(`[STDERR] ${result.stderr.slice(0,2000)}`);
      return { success: result.exitCode === 0, logs, envelope: result.envelope, stdout: result.stdout, stderr: result.stderr };
    } catch (e: any) {
      logs.push(`[ERROR] Approve failed: ${e.message}`);
      return { success: false, logs, envelope: null, stdout: e.stdout ?? '', stderr: e.stderr ?? '' };
    }
  }
